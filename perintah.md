...
- **Broker Flow** — per-ticker broker summary table (Buy/Sell/Net) with optional broker filter and MATCHED badge; plus **🔎 Cari Aktivitas per Broker** (`GET /brokers/activity`) — pilih kode broker (mis. SS) + periode (7/14/30/90 hari) + universe, lalu lihat di saham apa saja broker itu beli/jual/net + harga pasar terakhir; dan **📖 Direktori Broker** (`GET /brokers/directory`) — 88 broker IDX diklasifikasikan smart money (institusi/asing/BUMN/grup besar) vs ritel (aplikasi online/lokal) lengkap dengan afiliasi grup & emiten terkait (mis. DH→Sinarmas Group: BSDE/SMAR/TKIM; EP→MNC Group: MNCN/BMTR; CD→CT Corp: MEGA; SQ→Djarum/BCA: BBCA).
- **Klasifikasi di hasil scan (Layer B):** setiap baris broker di `top_buyers`/`top_sellers`/`matched_brokers` kini membawa `tags` (smart_money/retail/unknown), `category`, `group`, `issuers` — UI menampilkan chip 🧠 SMART / 🛒 RITEL + jumlah smart money buyer (`n_smart_money_buyers`).
- **Backtest pola akumulasi per broker** (`scripts/backtest_broker_accum.py`, data broker historis arjum + yfinance, disk-cache di `scripts/.broker_cache.json` supaya kuota arjum tidak terbakar saat run ulang): mengukur apakah minggu di mana broker X net buy memprediksi forward return lebih baik. Temuan awal (670 observasi, 20 ticker × 52 minggu): sinyal Layer A + SS net buy (moderat) lebih kuat (PF 1.25–1.33, win 56–59%) vs baseline PF 1.16–1.21; tapi **SS net buy sangat besar (>10 M/observasi) justru negatif (PF 0.3–0.8)** — hati-hati, aksi sangat besar bisa jadi distribusi. Catatan: kuota harian arjum (1000 req) terbatas — jalankan dengan `--freq 4` (hemat) dan pakai cache.
